...
// machineKey derives a 32-byte AES key from /etc/machine-id and the current user,
// binding encrypted secrets to this machine and user account.
func machineKey() ([32]byte, error) {
	id, err := os.ReadFile("/etc/machine-id")
	if err != nil {
		return [32]byte{}, fmt.Errorf("machine-id: %w", err)
	}
	user := os.Getenv("USER")
	return sha256.Sum256([]byte("TunnelDeck:" + strings.TrimSpace(string(id)) + ":" + user)), nil
}
